"""
VocaLift Utilities
Audio processing, feature extraction, ML inference, and analysis functions.
"""
...
def generate_feedback(coach_path, user_path, model, sr=SR):
    ...
    return {'prob_good': prob_good, 'prediction': prediction, 'overall_assessment': overall_assessment,
            'feedback_lines': feedback_lines, 'comp_feats': comp_feats}
...
def analyze_segments(coach_path, user_path, model, progress_callback=None, sr=SR, seg_len=SEG_LEN, hop=HOP):
    ...
    for i in range(num_segments):
        ...
        segment_results.append({'segment_num': i + 1, 'time_range': f"{start_time:.1f}s - {end_time:.1f}s",
                                'composite_score': composite_score, 'rating': rating,
                                'model_confidence': model_score * 100, 'mfcc_similarity': mfcc_score * 100,
                                'pitch_corr': pitch_score * 100, 'energy_corr': energy_score * 100,
                                'dtw_distance': comp_feats['dtw_mean'], 'tempo_diff': comp_feats['tempo_diff'],
...
def make_pitch_contour_figure(coach_path, user_path, sr=SR, fmin_note='C2', fmax_note='C7', hop_length=512, y_max=800):
    ...
    if len(t_c) and len(f0_c):
        fig.add_trace(go.Scatter(x=t_c, y=f0_c, mode='lines', name='Coach', line=dict(color=coach_color, width=2),
                                 hovertemplate="Coach<br>Time: %{x:.2f}s<br>Freq: %{y:.1f} Hz<extra></extra>"))
    if len(t_u) and len(f0_u):
        fig.add_trace(go.Scatter(x=t_u, y=f0_u, mode='lines', name='User', line=dict(color=user_color, width=2),
                                 hovertemplate="User<br>Time: %{x:.2f}s<br>Freq: %{y:.1f} Hz<extra></extra>"))
    ...
    for (s, e) in _voiced_intervals(voiced_mask_c, t_c):
        fig.add_shape(type="rect", x0=s, x1=e, xref="x", y0=0, y1=y_max, yref="y",
    ...
    fig.update_layout(title="Pitch Contour Comparison (Coach vs User)", xaxis_title="Time (s)", yaxis_title="Frequency (Hz)",
                      yaxis=dict(range=[0, y_max]), legend=dict(orientation="h", yanchor="bottom", y=1.04, xanchor="right", x=1, font=dict(color=text_color)),
                      margin=dict(l=40, r=20, t=60, b=40), template="plotly_dark", height=420,
